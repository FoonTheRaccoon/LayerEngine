...
namespace Logger
{
	...
	// Keep track of any shutdown functions needed for exit so we can call them in case of a fatal error
	inline Broadcaster<void()> shutdownBroadcaster;


	// Used to initialize logging file, set worker that will occasionally update the file, and hook up logger to an output. Returns true if successful.
	...
	// Updates log file.
	void AddToLogFile(const char* message);
	void AddToLogFile(const T_string& message);
...
#define LOG_VKRESULT(func)																				\
	...
	{																									\
		_PRINT_MESSAGE_INFO(LOG_SEVERITY_ERROR,															\
			T_string("VkResult == ", VkNames::GetVkResultName(_MACRO_CONCAT( vkresult, __LINE__ ))));	\
	}

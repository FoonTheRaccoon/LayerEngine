...
public:
	...
	// If nullptr is passed in then ALL functions that used a nullptr context will be removed from the function call array.
	// Any Register() call on a object's method should call Unregister() in it's destructor or sometime before it's destroyed.
	void Unregister(void* pContextObject)
	{
		auto new_end = std::remove_if(std::execution::par, m_FunctionsToBeCalled.begin(), m_FunctionsToBeCalled.end(),

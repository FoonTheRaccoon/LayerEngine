...
void VkSetup::CapturePhysicalDevice(VkRef& vkRef)
{
	...
	for (const auto& physicalDevice : physicalDevicesAvailable)
	{
		...
		if (_CheckPhysicalDeviceIsSuitableAndBuildReference(physicalDevice, vkRef.surface, physicalDeviceRef))
		{
			...
			availablePhysicalDevices.emplace_back(physicalDeviceRef);
		}
	}

	if (availablePhysicalDevices.empty()) { LOG_FATAL("No Suitable Vulkan Physical Device Found For Given Requirements!"); }

	// Check Suitable Devices and choose one (discrete GPU preferred)
	u32 suitableDevice = 0;
	for (u32 i = 0; i < availablePhysicalDevices.size(); i++)
	{
		if (availablePhysicalDevices[i].properties.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
		{
			suitableDevice = i;
			break;
		}
	...
	vkRef.phyDevice = availablePhysicalDevices[suitableDevice];
...
bool VkSetup::_CheckPhysicalDeviceSupportsDesiredFeatures(PhysicalDevice& phyDeviceReference)
{
	...
	for (u32 i = 0; i < numOfMembers; i++)
	{
		...
		{
				LOG_WARNING_MIN(T_string("Desired Device Feature ", VkNames::GetVkPhysicalDeviceFeaturesName(i),
...
endloop:
	...
	LOG_INFO(T_string(phyDeviceReference.properties.deviceName, " Preferred Surface Format: ", 
		VkNames::GetVkFormatName(phyDeviceReference.preferredSurfaceFormat.format),
		" / ", VkNames::GetVkColorSpaceKHRName(phyDeviceReference.preferredSurfaceFormat.colorSpace)));

	LOG_INFO(T_string(phyDeviceReference.properties.deviceName, " Preferred Present Mode: ",
		VkNames::GetVkPresentModeKHRName(phyDeviceReference.preferredPresentMode),
...
bool VkSetup::_CheckAndSetAttachmentFormats(PhysicalDevice& phyDeviceReference)
{
	...
	if (format32BitPack != VK_FORMAT_UNDEFINED)
	{
		...
		LOG_INFO(T_string(phyDeviceReference.properties.deviceName, " Preferred 32 Bit Pack Format: ", VkNames::GetVkFormatName(format32BitPack)));
	}
	else
	{
	...
	if (formatDepthStencil != VK_FORMAT_UNDEFINED)
	{
		...
		LOG_INFO(T_string(phyDeviceReference.properties.deviceName, " Preferred Depth Stencil Format: ", VkNames::GetVkFormatName(formatDepthStencil)));
	}
	else
	{
	...
	if (format16BitPack != VK_FORMAT_UNDEFINED)
	{
		...
		LOG_INFO(T_string(phyDeviceReference.properties.deviceName, " Preferred 16 Bit Pack Format: ", VkNames::GetVkFormatName(format16BitPack)));
	}
	else
	{
	...
	if (format10BitColor != VK_FORMAT_UNDEFINED)
	{
		...
		LOG_INFO(T_string(phyDeviceReference.properties.deviceName, " Preferred 10 Bit Color Format: ", VkNames::GetVkFormatName(format10BitColor)));
	}
	else
	{
